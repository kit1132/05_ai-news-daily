...
| リポ | パス | 更新時刻（JST目安） |
|---|---|---|
| `kit1132/01_ai-news-Master` | `digests/<YYYY>/<MM>/ai-news-<DATE>.md` | 毎朝 4:00 頃 |
| `kit1132/03_ai-news-industry` | `digests/<YYYY>/<MM>/ai-news-<DATE>.md` | 毎朝 5:00 頃 |
| `kit1132/02_ai-news-Copilot` | `digests/<YYYY>/<MM>/ai-news-<DATE>.md` | 毎朝 6:00 頃 |
...
1. ブランチを main に揃える:
   ...
   ```
2. 出力先 `daily/<YYYY>/ai-news-daily-<DATE>.md` が既に存在する場合は、何も変更せず「本日分は生成済み」と通知して終了する
3. 3ソースの当日ダイジェストを読み取る。取得できないソースは「欠損」として記録する
   - **3ソース全欠損**: 生成を中止し、その旨を通知して終了
   - **一部欠損**: 取得できたソースのみで生成し、冒頭の注記に欠損ソースを明記
4. 下記フォーマットに従い日次サマリーを生成し、`daily/<YYYY>/ai-news-daily-<DATE>.md` に保存する
5. `files.json` の配列先頭に新ファイルのパス（例: `daily/2026/ai-news-daily-2026-06-13.md`）を追加する
6. コミット & プッシュ:
   ```
